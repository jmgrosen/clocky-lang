...
pub fn translate<'a>(global_defs: &[GlobalDef<'a>]) -> Vec<u8> {
    ...
    for (i, def) in global_defs.into_iter().enumerate() {
        ...
        match def {
            GlobalDef::Func { rec, arity, env_size, body } => {
                let mut trans = FuncTranslator::new(global_defs, *rec, *env_size, *arity, &mut function_types, heap_global);
                ...
                exports.export(&format!("{}", i), wasm::ExportKind::Func, type_idx);
                codes.function(&func);
            },
            GlobalDef::ClosedExpr { body } => {
                let mut trans = FuncTranslator::new(global_defs, false, 0, 0, &mut function_types, heap_global);
                let ctx = trans.make_initial_ctx();
                trans.translate(ctx, body);
                let (type_idx, func) = trans.finish();
                functions.function(type_idx);
    ...
    for (arg, ret) in function_types.types {
...
impl<'a> FuncTranslator<'a> {
    fn new(globals: &'a [GlobalDef<'a>], rec: bool, env_size: u32, arity: u32, function_types: &'a mut FunctionTypes, heap_global: u32) -> FuncTranslator<'a> {
        FuncTranslator {
            globals,
            env_size,
            rec,
